...
if not df.empty:
    # التأكد من أن الأعمدة الرقمية كلها numbers
    ...
    # حساب current balance و total pnl بأمان
    if not df.empty:
...
with tabs[1]:
    if not df.empty:
        ...
        cols_h = st.columns(7)
        for i, d_n in enumerate(days): 
            cols_h[i].markdown(f"<p style='text-align:center; color:#8b949e'>{d_n}</p>", unsafe_allow_html=True)
        for week in cal:
            cols = st.columns(7)
...
with tabs[3]:
    if not df.empty:
        ...
        ca, cb = st.columns(2)
        with ca: 
            st.plotly_chart(px.scatter(df, x='rr', y='pnl', color='outcome', title="Risk Management", template="plotly_dark"), use_container_width=True)
        with cb: 
            st.plotly_chart(px.bar(df.groupby('mindset')['pnl'].sum().reset_index(), x='mindset', y='pnl', title="Psychology Tracker", template="plotly_dark"), use_container_width=True)
